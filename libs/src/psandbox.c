...
int release_psandbox(PSandbox *pSandbox) {
  ...
  success = (int)syscall(SYS_RELEASE_PSANDBOX, pSandbox->bid);
...
int wakeup_competitor(GList *competitors, PSandbox *sandbox) {
  ...
  for (iterator = competitors; iterator; iterator = iterator->next) {
    ...
    // Don't wakeup itself
    if (sandbox == competitor_sandbox || competitor_sandbox->activity->queue_state != QUEUE_SLEEP)
      continue;
    ...
    if ((float)delayed_tm > (float)(executing_tm - delayed_tm) *
        competitor_sandbox->delay_ratio * (float) g_list_length(competitors)) {
...
//      }
      ...
      if (event->key_size != 1) {
        ...
        for (iterator = competitors; iterator; iterator = iterator->next) {
          ...
          if (defer_tm >
              (executing_tm - defer_tm) * p_sandbox->delay_ratio * g_list_length(competitors)) {
            penalty_ns += defer_tm;
            delayed_competitors++;
          }
        }
...
time_t get_penalty(PSandbox *psandbox, GList *competitors) {
  ...
  for (iterator = competitors; iterator; iterator = iterator->next) {
    ...
    if (defer_tm >
        (executing_tm - defer_tm) * psandbox->delay_ratio * g_list_length(competitors)) {
      penalty_ns += defer_tm;
    }
  }
  return penalty_ns;
...
int is_interfered(PSandbox *psandbox, GList *competitors) {
  ...
  if (delayed_tm > (executing_tm - delayed_tm) *
      psandbox->delay_ratio * g_list_length(competitors)) {
    return 1;
  }
  return 0;
}
