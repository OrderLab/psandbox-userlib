...
def run_clang_format(clang_format_bin, changed_files, check_only):
    ...
    for changed_file in changed_files:
        if check_only:
            cmd = [clang_format_bin, "-style=file", "-output-replacements-xml", changed_file]
            print("Will check format on '{}'".format(changed_file))
        else:
            cmd = [clang_format_bin, "-style=file", "-i", changed_file]
            print("Will format '{}'".format(changed_file))
        # print("clang-format cmd = {}".format(cmd))
        try:
            cmd_output = subprocess.check_output(cmd)
            if check_only:
                if "replacement offset" in cmd_output:
                    print("ERROR: '{}' don't match format".format(changed_file))
                    ret = 1
                else:
                    print("Format OK for '{}'".format(changed_file))
        except subprocess.CalledProcessError as e:
...
def cli():
    ...
    parser.add_argument('--git-bin', type=str, default="git",
                        help="The git binary")
    parser.add_argument('file', nargs='*', help="Additional files to check")
